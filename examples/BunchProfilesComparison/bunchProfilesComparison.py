...
b_cos = bihc.Beam(
    Np=Np, bunchLength=bl, LPCfile=file, bunchShape="COS2", ppbk=ppbk, verbose=verbose
)
b_par = bihc.Beam(
    Np=Np,
    bunchLength=bl,
...
ax.legend()
fig.suptitle(
    "Bunch shape comparison in time for 1 bunch slot \n for same bunch length and filling scheme",
    fontweight="bold",
)
fig.tight_layout()
plt.show()
...
axs[0].text(
    0.3,
    0.8,
    f"Gaussian Power loss: {round(power_gauss, 2)} W",
...
axs[1].text(
    0.3,
    0.8,
    f"q-Gaussian Power loss: {round(power_qgauss, 2)} W",
...
axs[2].text(
    0.3,
    0.8,
    f"Binomial Power loss: {round(power_bin, 2)} W",
...
axs[3].text(
    0.3,
    0.8,
    f"Cosine Squared Power loss: {round(power_cos, 2)} W",
...
axs[4].text(
    0.3,
    0.8,
    f"Parabolic Power loss: {round(power_par, 2)} W",
