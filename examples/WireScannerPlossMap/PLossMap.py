...
pars = {
    1: [1.07e01, 12.24, 1.497e09],
    2: [3.06e01, 6.67, 1.920e09],
    3: [1.10e01, 6.59, 2.683e09],
    4: [4.13e00, 8.44, 3.297e09],
    5: [3.99e00, 66.55, 3.797e09],
    6: [4.20e00, 10.03, 4.059e09],
}
...
plt.plot(
    Z_origin.f / 1e9, np.real(Z_origin.Zr), lw=5, c="black", label="Origin", alpha=0.9
)
...
plt.yticks(fontsize=fontsize, color=ax_color)
...
def calModePower(frequency, par, index: int):
    ...
    Z.getResonatorImpedance(
        Rs=Resonator_par[0], Qr=Resonator_par[1], fr=Resonator_par[2]
    )  # input the resonator parameters from IDDEFIX
    fmax = np.max(Z.f)

    beam_qgauss = bihc.Beam(
        bunchLength=bunch_length,
        Np=N_density,
        bunchShape="q-GAUSSIAN",
        qvalue=q_value,
...
for i in range(mode_number):
    ...
    d = pd.read_csv(
        Pmonitors[i], skiprows=[0, 1], sep=r"\s+", header=None, low_memory=False
    )
    d[d == "-nan(ind)"] = 0
    x, y, z, PowerMap = (
        np.array(d[0]),
...
Radius = 8
fieldonsurf = surf_ab.interpolate(
    pdata,
    radius=Radius,
    sharpness=2,
)
