...
blft = (
    7.505192141958421e-10 * 4
)  # 0.225 m / clight *4 (to fit Francesco's definition: -2sigma, +2sigma)
bl = blft * 2
...
b1 = bihc.Beam(
    ...
    machine="SPS",
    fillMode="FB",
    fillingScheme=fillingSchemeSPS(1),
    Np=Np,
    t0=t0,
    fmax=fmax,
)  # first injection
b2 = bihc.Beam(
    ...
    machine="SPS",
    fillMode="FB",
    fillingScheme=fillingSchemeSPS(2),
    Np=Np,
    t0=t0,
    fmax=fmax,
)  # second injection
b3 = bihc.Beam(
    ...
    machine="SPS",
    fillMode="FB",
    fillingScheme=fillingSchemeSPS(3),
    Np=Np,
    t0=t0,
    fmax=fmax,
)  # third injection
b4 = bihc.Beam(
    ...
    machine="SPS",
    fillMode="FB",
    fillingScheme=fillingSchemeSPS(4),
    Np=Np,
    t0=t0,
    fmax=fmax,
)  # 4th injection flat bottom
b4ft = bihc.Beam(
    bunchLength=blft,
    machine="SPS",
    fillMode="FLATTOP",
...
print(f"Computed avg. power loss: {avg_powerloss} W")
print(f"Computed max. power loss: {b4ft.getPloss(Z)[0]} W")

# Plotting
fig, (axs) = plt.subplots(4, 1, figsize=(4, 6))

axs[3].plot(
    b4.longitudinalProfile[0] * 1e9, b4.longitudinalProfile[1], label="4 trains"
)
axs[2].plot(
    b3.longitudinalProfile[0] * 1e9, b3.longitudinalProfile[1], label="3 trains"
)
axs[1].plot(
    b2.longitudinalProfile[0] * 1e9, b2.longitudinalProfile[1], label="2 trains"
)
...
ax2.plot(b4ft.powerSpectrum[0] / 1e9, b4ft.spectrum[1], label="4 trains flat top")
...
axx2.text(
    0.5,
    0.2,
    f"Max Power loss: {round(b4ft.getPloss(Z)[0], 2)} W",
...
axx2.text(
    0.5,
    0.1,
    f"Avg Power loss: {round(avg_powerloss, 2)} W",
    transform=axx2.transAxes,
    color="k",
    weight="bold",
)
