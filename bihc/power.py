...
class Power:
    ...
    def getShiftedPloss(self, Z, shift=20e6):
        ...
        for step in tqdm(shifts, "Computing scan: ", total=2 * size):
            ...
            if step > 0:
                Zmod.Zr[:step] = 0.0
            if step < 0:
                Zmod.Zr[: 2 * size - step] = 0.0
            power = np.append(power, self.getPloss(Zmod)[0])

        Zmod.f, Zmod.Zr, Zmod.Zi = f, Zint, Zint * 0.0
        self.Z = Zmod.copy()
    ...
    def get2BeamPloss(
        ...
        if tau_s is not None:
            ...
            raise Exception(
                "Specify s (distance from IP) or tau_s (phase shift of the two beams)"
            )
        ...
        if Z_1 is not None and (offset1 is not None or offset2 is not None):
            ...
            P_loss = []
            for i, shift in tqdm(
                enumerate(tau_s), "Computing 2-beam power with offset: "
            ):
                P_density = (
                    P
                    * (Zreal_0 + (offset1[i] + offset2[i]) * Zreal_1)
                    * (1 - np.cos(2 * np.pi * f * shift))
                )
