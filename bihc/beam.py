...
class Beam(Impedance, Power, Plot):
    ...
    def bunchShape(self, newShape):
        ...
        if newShape in shapeList:
            ...
            raise ValueError(
                "bunchShape should be: 'GAUSSIAN', 'BINOMIAL', 'PARABOLIC', 'q-GAUSSIAN', or 'COS2'."
            )
    ...
    def spectrum(self):
        ...
        if self._isSpectrumReady:
            ...
            if self._spectrumtype == "numeric":
                ...
                fc = (
                    1 / deltaT
                )  # in frequency we have a periodic signal of period fc where fc is 1/deltaT where the sampling step
                ...
                deltaF = fc / len(s)
                f = np.linspace(
                    -fc / 2, fc / 2 - deltaF, len(S)
                )  # vector of K point from min_value to max_value (Domain in frequency)

            else:  # analytic formula (C.Zannini)
                print(
                    "\033[93m"
                    + "Warning -> Analytic FFT might take a long time to be computed"
                    + "\033[0m"
                )
                ...
                if self._bunchShape == "BINOMIAL":  # todo
                    raise Exception(
                        "BINOMIAL is not supported for analytic spectrum calculation"
                    )
                    ...
                    for p in progressbar(
                        ...
                        CosSin = np.sqrt(5) * sigmapar * p * wrev / c * np.cos(
                            np.sqrt(5) * sigmapar * p * wrev / c
                        ) - np.sin(np.sqrt(5) * sigmapar * p * wrev / c)
                        lambdas[p] = (
                            -3
                            * c**3
                            / (
                ...
                f = (
                    np.linspace(1, len(S), len(S)) * self.frev
                )  # [TODO] should it start in 0?
    ...
    def _setBunches(self):
        ...
        if (self.realMachineLength) and (self.t0 * self.M > self.T_1_TURN):
            ...
            if self.verbose:
                print(
                    "t0 has been resized to ",
                    self.t0,
                    " because it was to big to fill ",
                    self.M,
                    " buckets in the real machine length",
                )
        ...
        for i in range(self.M):
            if self._fillingScheme[i]:
                ...
                if self._bunchShape == "BINOMIAL":
                    ...
                    sTemp = (
                        1
                        / (self._bunchLength[i] * np.sqrt(2 * np.pi))
                        * (np.e)
                        ** (
                            -((tTemp - self.phi[i]) ** 2)
                            / (2 * self._bunchLength[i] ** 2)
                        )
                    )  # Gaussian function
                    mask = (np.abs(tTemp - self.phi[i])) < self.l
        ...
        if self.verbose:
            print(
                ...
                np.mean(self._bunchLength[mask]) * 4,
                "s",
            )
    ...
    def setCustomBeam(self):
        ...
        self._bunchLength[self._fillingScheme] = (
            self.BUNCH_LENGTH_GLOBAL
    ...
    def setCustomBeamWithFillingScheme(self):
        ...
        if len(self._fillingScheme) > self.M:
            sys.exit(
                "ERROR : the length of the fillingScheme exceed the slot that you set (M)"
            )
        ...
        for j in range(self.M):
            if self._fillingScheme[j]:
                ...
                if type(self.BUNCH_LENGTH_GLOBAL) is float:
